...
class RecordViewModel(application: Application) : AndroidViewModel(application) {
        ...
        with(list) {
            ...
            when (record) {
                ...
                is RecordCipher -> {
                    ...
                    record.aad?.displayEntries()?.forEach {
                        val title = R.string.record_aad_format.string(it.key.displayName, "")
                        add(RDItem(title = title, content = it.value))
                    }
                    record.rawData.displayEntries().forEach {
                        val title = R.string.record_raw_data_format_type.string(it.key.displayName)
    ...
    private fun Map<RecordValueType, String>.toAadList(): List<String> {
        return displayEntries().map {
            R.string.record_aad_format.string(it.key.displayName, it.value)
        }
    }
