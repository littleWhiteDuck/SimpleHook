package me.simpleHook.core.ui.custom
...
abstract class CustomSwipeCardViewGroup(context: Context, attrs: AttributeSet?) :
     ...
     * @param ios
     */
    fun setIos(ios: Boolean): CustomSwipeCardViewGroup {
        isIos = ios
        return this
    }
     ...
     * @return
     */
    fun setLeftSwipe(leftSwipe: Boolean): CustomSwipeCardViewGroup {
...
/*                    if (!mScroller.isFinished()) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    if (!iosInterceptFlag) { //且滑动了 才判断是否要收起、展开menu
                        ...
                        if (Math.abs(velocityX) > 1000) { //滑动速度超过阈值
                            ...
                            val mLimit = mRightMenuWidths * 4 / 10
    ...
    fun smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        viewCache = this@CustomSwipeCardViewGroup
     ...
     * 平滑关闭
     */
    fun smoothClose(delayMills: Long = 0) {
...
/*        mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0);
        ...
        mCloseAnim?.startDelay = delayMills
    ...
    companion object {
        ...
        var viewCache: CustomSwipeCardViewGroup? = null
            private set
    ...
    protected fun View.autoMeasure() {
        measure(
            this.defaultWidthMeasureSpec(this@CustomSwipeCardViewGroup),
            this.defaultHeightMeasureSpec(this@CustomSwipeCardViewGroup)
        )
    }
    ...
    protected fun View.autoLayout(
        ...
        if (fromRight) {
            val xPosition = this@CustomSwipeCardViewGroup.measuredWidth - x - measuredWidth
            if (fromBottom) {
                autoLayout(
                    xPosition, this@CustomSwipeCardViewGroup.measuredHeight - y - measuredHeight
                )
            } else {
            ...
            if (fromBottom) {
                autoLayout(x, this@CustomSwipeCardViewGroup.measuredHeight - y - measuredHeight)
            } else {
