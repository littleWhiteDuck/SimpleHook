...
class AssistItemView(context: Context) : MaterialCardView(context) {
    ...
    class ContainerView(context: Context): CustomViewGroup(context){
        ...
        val versionName = AppCompatTextView(ContextThemeWrapper(context, R.style.text_view_caption)).apply {
        ...
        override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
            ...
            appName.measure((measuredWidth - paddingStart - paddingEnd).toExactlyMeasureSpec(), appName.defaultHeightMeasureSpec(this))
            icon.autoMeasure()
            val remainWidth = measuredWidth - paddingStart - paddingEnd -versionName.marginEnd- icon.measuredWidth
            versionName.measure(remainWidth.toExactlyMeasureSpec(), versionName.defaultHeightMeasureSpec(this))
            setMeasuredDimension(measuredWidth, paddingTop + appName.measuredHeight + icon.measuredHeightWithMargins + paddingBottom)
        }
        ...
        override fun onLayout(p0: Boolean, p1: Int, p2: Int, p3: Int, p4: Int) {
            ...
            icon.autoLayout(paddingEnd, measuredHeight - icon.measuredHeightWithMargins - paddingBottom, fromRight = true)
        }
    }
    }
