 ...
 */
fun Method.hookMethod(hookCallback: XC_MethodHook): Unhook {
    return XposedBridge.hookMethod(this, hookCallback)
}

fun Constructor<*>.hookMethod(hookCallback: XC_MethodHook): Unhook {
    return XposedBridge.hookMethod(this, hookCallback)
}
 ...
 * 执行数组中所有的unhook
 */
fun Array<Unhook>.unhookAll() {
    this.forEach { it.unhook() }
}

fun Iterable<Unhook>.unhookAll() {
    this.forEach { it.unhook() }
}
